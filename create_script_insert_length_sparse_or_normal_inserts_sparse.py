#!/usr/bin/env python
"""
Create a cluster job file to combine the average insert length sparsity
and normal insert coverage depth sparsity discriminator tracks.
"""
...
usage: create_script_insert_length_sparse_or_normal_inserts_sparse [options] > insert_length_sparse_or_normal_inserts_sparse.sh
  ...
  --track=<filename>         (required) track file to create
                             (default is {base}/tracks/{run}.insert_length_sparse_or_normal_inserts_sparse)
  --tempinput=<filename>     temporary file to hold an input track file, if
                             needed; only needed if the second input track was
                             gzipped
                             (default is {base}/tracks/{run}.insert_length_sparse_or_normal_inserts_sparse.scratch)
  --temp=<filename>          temporary file to hold track file, if needed; only
                             needed if --gzip and --bigwig are both used
                             (default is {base}/tracks/{run}.insert_length_sparse_or_normal_inserts_sparse.temp)
...
def main():
	...
	if (trackName == None):
		trackName = "{base}/tracks/{run}.insert_length_sparse_or_normal_inserts_sparse"
	...
	trackId = "%s.insert_length_sparse_or_normal_inserts_sparse" % runName
	...
	if (bigWigFilename != None):
		...
		description = "sparse intervals in average insert lengths OR sparse intervals in normal inserts"
		if (dateInTrackname): description += " (${today})"

		commands =  []
		command  =  ["make_bigwig_info"]
		command  += ["--url=%s" % bigWigUrl]
		command  += ["--name=\"%s short average OR normal depth sparse\"" % runName]
